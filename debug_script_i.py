...
# Let PyTorch run any operation Apple's MPS backend lacks on the CPU instead
...
# ── Layout label → recognition class (i) ─────────────────────────────────────
# Surya's layout label decides how a region is read:
#   "display" — large, bold display type (headlines, masthead);
#   "body"    — small, often worn text type (everything else that is text);
#   "skip"    — photographs and figures; not read (the sweep recovers any
#               text printed inside them).
# A label Surya emits that is not listed here is read as UNKNOWN_LABEL_CLASS.
LABEL_CLASS: Dict[str, str] = {
    "Section-header":    "display",   # column and article headlines
    "Page-header":       "display",   # masthead, volume / issue / date strip
    "Text":              "body",      # body copy — primary article content
    "Caption":           "body",      # photo / illustration captions
    "Footnote":          "body",      # editorial notes, source citations
    "List-item":         "body",      # bulleted or numbered list entries
    "Page-footer":       "body",      # pagination lines, print datelines
    "Table-of-contents": "body",      # index entries
    "Handwriting":       "body",      # editorial annotations, marginalia
    "Text-inline-math":  "body",      # inline notation within prose
    "Formula":           "body",      # display equations
    "Table":             "body",      # stock quotes, box scores, schedules
    "Form":              "body",      # coupons, order forms, notices
    "Picture":           "skip",
    "Figure":            "skip",
}
UNKNOWN_LABEL_CLASS = "body"
SKIP_LABELS = {lbl for lbl, cls in LABEL_CLASS.items() if cls == "skip"}

# How each recognition class is read.
#   model      — TrOCR checkpoint (identical names are loaded once).
#                large_handwritten has performed best on degraded historic
#                body type; large_printed is worth an A/B test for "display",
#                which is clean, bold, printed type.  Two different large
#                checkpoints roughly double TrOCR's memory use.
#   detect_on  — image the Surya line detector runs on:
#                "processed" = the tile-preprocessed page (sharpened, good
#                              for separating tightly set body lines);
#                "raw"       = an autocontrasted crop of the raw render
#                              (the unsharp mask blobs bold strokes together).
#   split_wide — read very wide, tall lines in overlapping segments
#                (_trocr_read_wide_crop), avoiding TrOCR's square-resize squash.
# Recognition crops for every class come from the raw render, autocontrasted.
RECOGNITION_CLASSES: Dict[str, Dict] = {
    "display": {"model": TROCR_MODELS["large_handwritten"],
                "detect_on": "raw",       "split_wide": True},
    "body":    {"model": TROCR_MODELS["large_printed"],
                "detect_on": "processed", "split_wide": False},
}

# "Page furniture": headline/masthead-type content.  Breaks ties between two
# reads of the same patch of page in dedupe_overlapping_elements().
FURNITURE_LABELS = {"Section-header", "Page-header", "Page-footer", "Table-of-contents"}
...
SPARSE_LINE_WIDTH_RATIO          = 2.0    # width / (height * char count) above this = noise
...
# ── Recognition crops (i) ─────────────────────────────────────────────────────
# Padding around each detected line, in multiples of the line's own height,
# so it scales with the type: about 6×3 px on 300 DPI body text, proportionally
# more on headlines.  Padding never extends past the Surya region the line was
# detected in (the detected line itself is never cut), so a neighbouring
# column, rule or headline cannot leak into the crop.
PAD_X_EM = 0.20
PAD_Y_EM = 0.10
CROP_AUTOCONTRAST_CUTOFF = 1       # cutoff % for the per-crop autocontrast

# Normalise TrOCR's IAM-style spacing around punctuation
# ('Moscow , Wednesday' → 'Moscow, Wednesday'; '( alas' → '(alas').
NORMALIZE_PUNCT_SPACING = True

# ── Selective beam search ─────────────────────────────────────────────────────
# Every crop is decoded greedily first.  A beam retry happens only when ALL hold:
...
# Device for TrOCR: "auto" = CUDA if present, else Apple MPS, else CPU.
# Or force "cuda" / "mps" / "cpu".
TROCR_DEVICE = "auto"

# (i) Layout attempts per page.  Accelerator memory is freed between
# attempts; if every attempt raises (e.g. MPS out of memory) the FILE is
# marked FAILED and no output is written, so it is retried on the next run
# instead of being saved with a layout-less, column-mixing text layer.
LAYOUT_ATTEMPTS = 2
...
SWEEP_COVERAGE_PAD   = 6      # px of slack when checking coverage
...
# A cluster of recovered lines in an unclaimed gap is only relocated in
# reading order (treated as a whole missed column) if it clears BOTH of these.
...
# lines overlap in y by more than ROW_OVERLAP_FRACTION of the smaller line's
# height AND overlap in x by less than ROW_MAX_X_OVERLAP (see _assign_visual_rows).
ROW_OVERLAP_FRACTION = 0.5
ROW_MAX_X_OVERLAP    = 0.5

# ── Banner/masthead reading-order correction ──────────────────────────────────
BANNER_BAND_OVERLAP_THRESHOLD = 0.18   # y-overlap (fraction of the narrower region) = same band
BANNER_BAND_MIN_GAP_PX        = max(10, round(DPI * 0.06))  # smaller gaps still = same band

# ── Duplicate removal ─────────────────────────────────────────────────────────
DEDUPE_CONTAINMENT      = 0.5
DEDUPE_MAX_AREA_RATIO   = 3.0
# In a duplicate pair where exactly one read is page furniture, the furniture
# read wins UNLESS the other read is more confident by more than this margin.
DEDUPE_FURNITURE_MARGIN = 0.15

# ── Wide display lines ────────────────────────────────────────────────────────
MAX_HEADER_AR          = 6.0   # width / height above which a display line is split
HEADER_SEGMENT_OVERLAP = 0.20  # fraction of segment width shared with the next segment
HEADER_SPLIT_MIN_H     = round(DPI * 0.30)   # only lines at least this tall are split
...
#         overwritten by every page.  False → separate files for every page.
...
#                      Fallback: PyMuPDF's built-in "helv" substitute, which
#                      TextWriter also embeds (unsubset) — still PDF/A-safe.
...
# printed line's box.  The clamp only guards against degenerate input.
MIN_TEXT_STRETCH = 0.05
MAX_TEXT_STRETCH = 20.0

# ── Article threads ───────────────────────────────────────────────────────────
# Surya's reading order as a native PDF article thread: one thread per page,
# one bead per reading-order block.  Pre-existing threads are replaced.
...
def preprocess_newspaper(image: Image.Image) -> Image.Image:
    ...
    then applies a 1.5 px unsharp mask to sharpen hairline serifs.

    Used for layout analysis and for line DETECTION in "body" regions and the
    sweep.  Recognition never reads this image (see _recognition_crop): the
...
def autocontrast_crop(crop: Image.Image) -> Image.Image:
    """
    (i) The single crop-level preprocessing step: one autocontrast over the
    whole crop, cut from the RAW render.  No tiling (so no seams) and no
    unsharp mask (which blobs bold display strokes and which TrOCR, trained
    on natural-looking text images, doesn't benefit from).  Replaces h's
    preprocess_header_crop and preprocess_line_crop, which were identical.
    """
    try:
        gray = crop.convert("L")
        gray = ImageOps.autocontrast(gray, cutoff=CROP_AUTOCONTRAST_CUTOFF)
        return gray.convert("RGB")
    except Exception as exc:
        logger.warning(f"Crop preprocessing fallback: {exc}")
        return crop.convert("RGB")


def _recognition_crop(
    raw_image: Image.Image,
    box: List[float],
    scope: Optional[List[float]] = None,
) -> Image.Image:
    """
    Cut the image TrOCR will read for `box` ([x0, y0, x1, y1], absolute page
    pixels) from the raw render, and autocontrast it.

    (i) Padding is PAD_X_EM / PAD_Y_EM times the box's own height, so it
    scales with the type size.  If `scope` (the Surya region the line was
    found in) is given, the padding stops at the region's edge, but the
    detected line itself is never cut, even where it pokes outside the region.
    Pass the region as both `box` and `scope` to read exactly the region.
    """
    iw, ih = raw_image.size
    lh     = max(1.0, box[3] - box[1])
    px, py = PAD_X_EM * lh, PAD_Y_EM * lh
    x0, y0, x1, y1 = box[0] - px, box[1] - py, box[2] + px, box[3] + py
    if scope is not None:
        x0 = max(x0, min(scope[0], box[0]))
        y0 = max(y0, min(scope[1], box[1]))
        x1 = min(x1, max(scope[2], box[2]))
        y1 = min(y1, max(scope[3], box[3]))
    x0, y0 = max(0, int(x0)), max(0, int(y0))
    x1, y1 = min(iw, int(round(x1))), min(ih, int(round(y1)))
    return autocontrast_crop(raw_image.crop((x0, y0, x1, y1)))
...
def setup_pdfa_resources() -> bool:
    ...
    Make sure the two PDF/A resources exist locally:
      ...
      srgb.icc            — copied from the OS if present, otherwise generated
                            locally with Pillow.  An existing srgb.icc that is
                            not a real ICC profile is detected and replaced.
    """
    ok = True
...
def _article_bead_boxes(
    ...
    uses that region's box, otherwise (a relocated column of recovered lines)
    the union of the group's line boxes.  Regions that yielded no text get no
    bead.  Conversion to PDF user space happens later (_frac_to_pdf_rect).
    """
    iw, ih = img_size
...
def _frac_to_pdf_rect(
    ...
    into PDF user-space coordinates [x0, y0, x1, y1] (origin bottom-left),
    using the page's CropBox and /Rotate.  Written out explicitly because
    PyMuPDF's derotation / transformation matrices do not combine correctly
    for rotated pages whose CropBox is offset from the MediaBox origin.
    """
...
def add_article_threads(pdf: "pikepdf.Pdf", thread_specs: Dict[int, Dict]) -> int:
    ...
    Existing threads and page /B arrays are removed first.  Returns the
...
def setup_pdfa_compliance(pdf_path: str, thread_specs: Optional[Dict[int, Dict]] = None) -> None:
    ...
    Post-save pass with pikepdf.  MUST run after the file is on disk.
      ...
      - writes the article threads (ARTICLE_THREADS_ENABLED);
...
# (i) recognition class → (processor, model); classes sharing a checkpoint
# share the same objects.
Recognizers = Dict[str, Tuple[TrOCRProcessor, VisionEncoderDecoderModel]]


def _select_device() -> "torch.device":
    """TROCR_DEVICE, or for "auto": CUDA → Apple MPS → CPU."""
...
def free_accelerator_memory() -> None:
    """(i) Release cached GPU/MPS memory (PyTorch keeps it between pages otherwise)."""
    gc.collect()
    try:
        if torch.cuda.is_available():
        ...
        logger.debug(f"  Could not empty accelerator cache: {exc}")


class LayoutFailedError(RuntimeError):
    """(i) Layout analysis raised on every attempt; the file must not be saved."""


def _device_smoke_test(processor: "TrOCRProcessor", model: "VisionEncoderDecoderModel") -> bool:
    """
    Run one greedy and one beam decode on a blank image on the model's
...
def load_recognizers() -> Recognizers:
    """
    (i) Load one TrOCR checkpoint per distinct name in RECOGNITION_CLASSES and
    map every recognition class to its (processor, model).  With the default
    configuration both classes share one model, exactly as in h.
    """
    device = _select_device()
    by_name: Dict[str, Tuple[TrOCRProcessor, VisionEncoderDecoderModel]] = {}
    for cls, spec in RECOGNITION_CLASSES.items():
        name = spec["model"]
        if name not in by_name:
            logger.info(f"  TrOCR processor + model: {name} …")
            processor = TrOCRProcessor.from_pretrained(name)
            model     = VisionEncoderDecoderModel.from_pretrained(name)
            model.to(device)
            model.eval()
            by_name[name] = (processor, model)

    if device.type != "cpu" and not all(_device_smoke_test(p, m) for p, m in by_name.values()):
        logger.warning(f"  TrOCR generation failed on {device}; falling back to CPU.")
        device = torch.device("cpu")
        for _, m in by_name.values():
            m.to(device)
    logger.info(f"  TrOCR device: {device}")

    return {cls: by_name[spec["model"]] for cls, spec in RECOGNITION_CLASSES.items()}
...
def load_models():
    ...
    TrOCR  — one checkpoint per recognition class (see RECOGNITION_CLASSES)
    ...
    # ── Surya performance / memory tuning (callers may override via env) ─────
    ...
    return det_predictor, layout_predictor, recognizers
...
# Case- and underscore-insensitive lookup onto the canonical labels.
_CANON_LABELS: Dict[str, str] = {
    c.lower().replace("_", "-"): c for c in LABEL_CLASS
}
...
def _label_class(label: str) -> str:
    """(i) Recognition class ("display", "body" or "skip") for a canonical label."""
    return LABEL_CLASS.get(label, UNKNOWN_LABEL_CLASS)
...
def _nms_regions(regions: List[Dict], iou_threshold: float = 0.45) -> List[Dict]:
    """
    Non-maximum suppression over layout regions of the SAME label (largest
    box wins).  Boxes with different labels are never suppressed against
    each other: a Picture box must not be able to delete an overlapping
    Text box.
    """
    if not regions:
        return regions
...
def _band_merge_test(lo: float, hi: float, glo: float, ghi: float,
    ...
    they overlap by more than `overlap_threshold` of the narrower interval,
    or they don't overlap but the gap between them is smaller than `min_gap`.
    """
...
def _group_by_y_band(
    ...
    # A region processed out of y0 order can retroactively bridge two groups;
...
def _reorder_banner_regions(layout_regions: List[Dict]) -> None:
    ...
    occasionally mis-scores.

    Finds the page's top masthead band (a band entirely above the body band
    that contains at least one "display"-class region), then re-anchors only
    the masthead-band members whose position doesn't already sort before
    the body.  Every other region is left exactly as Surya gave it.
    ...
    for y0, y1, members in bands:
        if members is body_members:
            continue
        if y1 <= body_y0 and any(_label_class(r["label"]) == "display" for r in members):
    ...
    for r in misplaced:
        ...
        logger.info(
            f"  [ORDER] moved {r['label']} bbox=({r['bbox'][0]:.0f},{r['bbox'][1]:.0f}"
            f"→{r['bbox'][2]:.0f},{r['bbox'][3]:.0f}) from position {old} to "
            f"{r['position']} -- beside the nearest masthead piece "
            f"(pos={nearest['position']}), not in the column flow."
        )
...
def parse_layout_result(
    ...
    Surya may run layout at a lower internal resolution; `result.image_bbox`
    gives that coordinate space, and boxes are rescaled back to the size of
    the image passed in (`orig_image_size`).  Each region dict carries bbox,
    polygon, canonical label, Surya's reading-order position and top_k
    alternative labels.  Same-label duplicates are removed with NMS.
...
def _sequence_confidence(out, model: VisionEncoderDecoderModel,
    ...
    """
    Mean per-token probability of the RETURNED sequence (0–1), via
    model.compute_transition_scores, which follows the chosen path through
    beam search as well as greedy decoding.  Only tokens actually in the
    returned sequence are averaged (decoder-start skipped, beam padding after
    the first EOS ignored).
    """
...
def _content_key(text: str) -> str:
    """Text reduced to lower-case letters and digits, for spotting cosmetic-only changes."""
...
def _trocr_read(
    ...
    Greedy decoding first.  A beam retry is attempted only when
    max(BEAM_RETRY_ABOVE, min_conf) <= greedy confidence < BEAM_RETRY_BELOW
    and the greedy text has >= BEAM_MIN_CHARS non-space characters.  The
    beam result is adopted only if it is non-empty, at least as confident,
    and differs from greedy in more than case, spacing or punctuation.

    Returns (text, confidence) with punctuation spacing normalised.
    """
    try:
...
def _join_segments(texts: List[str]) -> str:
    ...
    word at each boundary (a side effect of the overlap between segments in
    _trocr_read_wide_crop).  Comparison ignores case and surrounding punctuation.
    """
    out: List[str] = []
    for t in texts:
        words = t.split()
...
def _trocr_read_wide_crop(
    ...
    """
    OCR a display-type crop that may be much wider than it is tall (a
    masthead or banner headline).

    TrOCRProcessor resizes every crop to a fixed square, compressing the
    letterforms of a very wide crop horizontally.  Crops wider than `max_ar`
    AND at least HEADER_SPLIT_MIN_H tall are split into overlapping segments,
    read independently and joined; everything else is read whole.
    Confidence is the mean across segments that returned text.
    """
    w, h = crop.size
...
def _is_noise(
    ...
    punctuation noise, or a confidence-floor rejection.  `min_conf` lets
    callers apply a stricter floor (the recall sweep uses SWEEP_MIN_CONFIDENCE).
    """
    if not text:
        return True
    ...
    for pat in noise_pats:
        if re.match(pat, tc) and confidence < SINGLE_CHAR_CONFIDENCE_THRESHOLD:
            return True
    # Vowel-less test applies only to purely alphabetic strings.
    if (tl > 3 and tc.isalpha()
            and not any(c in "aeiouy" for c in tc.lower())
            and confidence < 0.7):
        return True
    # A box far wider than its recognised text would need at that height is
    # usually a mostly-blank strip TrOCR filled in with a plausible guess.
...
def _make_element(text: str, box: List[float], confidence: float,
                  label: str, position: float) -> Dict:
    """
    (i) One OCR element.  font_size (render pixels) follows the box height
    with no upper cap, so the invisible text on a large headline is as tall
    as the printed type (h capped it at 72 px ≈ 17 pt).
    """
    return {
        "text":             text,
        "bbox":             [float(v) for v in box],
        "confidence":       confidence,
        "font_size":        max(6.0, (box[3] - box[1]) * 0.85),
        "source_label":     label,
...
def ocr_region(
    ...
    """
    OCR one layout region, read according to its recognition class.

    The region's Surya label selects a class in RECOGNITION_CLASSES, which
    decides:
      - the detection image — the tile-preprocessed page ("processed", body
        type) or an autocontrasted crop of the raw render ("raw", display type);
      - whether wide, tall lines are read in segments (split_wide);
      - which TrOCR checkpoint reads the lines.

    Lines found by the detector are each read from a raw-render crop padded
    in proportion to the line height, with the padding kept inside the region
    (_recognition_crop).  (i) Only if the detector finds no usable line is
    the whole region read as one crop, exactly at Surya's region box.  Any
    lines the detector misses in an otherwise-detected region are left to
    the recall sweep.
    ...
    spec            = RECOGNITION_CLASSES[cls]
    processor, model = recognizers[cls]
    ...
    if spec["detect_on"] == "raw":
        det_crop = autocontrast_crop(raw_image.crop((x0, y0, x1, y1)))
    else:
        det_crop = page_image.crop((x0, y0, x1, y1))

    if spec["split_wide"]:
        read = lambda img: _trocr_read_wide_crop(img, processor, model)
    else:
        read = lambda img: _trocr_read(img, processor, model)

    # ── Detected lines ────────────────────────────────────────────────────────
    elems: List[Dict] = []
    for lb in _surya_line_bboxes(det_crop, det_predictor):
        ...
        text, confidence = read(_recognition_crop(raw_image, abs_bbox, scope=region_box))
        if _is_noise(text, confidence, lh, lw):
            logger.debug(f"      [NOISE] conf={confidence:.2f} {lw}×{lh}px | {text[:40]}")
            continue
        elems.append(_make_element(text, abs_bbox, confidence, label, region["position"]))

    if elems:
        logger.debug(f"      {cls}: {len(elems)} line(s)")
        return elems

    # ── Fallback: whole region as one crop ────────────────────────────────────
    text, confidence = read(_recognition_crop(raw_image, region_box, scope=region_box))
    if _is_noise(text, confidence, rh, rw):
        logger.debug(f"      {cls}: no lines detected; whole-region read rejected")
        return []
    logger.debug(f"      {cls}: no lines detected; whole-region read {len(text)} char(s)")
    return [_make_element(text, region_box, confidence, label, region["position"])]
...
def _coverage(box: List[float], elements: List[Dict], pad: float = 0.0) -> float:
    """
    Fraction of `box` already lying inside existing elements (0–1), with each
    existing box grown by `pad` px to absorb small detector disagreements.
    """
...
def _column_bands(layout_regions: List[Dict], page_w: float) -> List[List[float]]:
    ...
    every other region, however short, claims its own x-range.  A column the
    layout model skipped entirely leaves a wide gap between bands.
    """
    ivals = [
...
def _assign_recovered_positions(
    ...
    Lines centred in an x-gap between column bands are grouped into columns
    by x-overlap; a group clearing both MIN_UNCLAIMED_COLUMN_LINES and
    MIN_UNCLAIMED_COLUMN_HEIGHT_FRAC is placed just after the bands to its
    left.  Everything else keeps its nearest-region position.  Positions may
    be fractional.  Returns the number of lines re-positioned.
    """
...
def _assign_visual_rows(elements: List[Dict]) -> None:
    ...
    Within each reading position, each line (sorted top-to-bottom) is
    compared only to the previous one.  Two lines share a row only if they
    overlap substantially in y (> ROW_OVERLAP_FRACTION of the smaller height)
    AND sit mostly side by side (x-overlap < ROW_MAX_X_OVERLAP).
    """
...
def dedupe_overlapping_elements(elements: List[Dict]) -> List[Dict]:
    """
    Remove an element that is a duplicate re-read of another element's area.

    A pair counts as a duplicate if one box is contained in the other by more
    than DEDUPE_CONTAINMENT of its own area and the areas are comparable
    (DEDUPE_MAX_AREA_RATIO).  If exactly one of the pair is page furniture
    (FURNITURE_LABELS), it is kept unless the other read is more confident
    by more than DEDUPE_FURNITURE_MARGIN; otherwise the lower-confidence
    read is dropped.
    """
    if len(elements) < 2:
        return elements
    ...
    for oi, i in enumerate(order):
        ...
        for j in order[oi + 1:]:
            if dropped[j]:
                continue
            b = elements[j]
            if b["bbox"][1] > ay1:      # sorted by y0 — nothing further below can overlap 'a'
                break
...
def _split_wide_sweep_line(box: List[float], bands: List[List[float]]) -> List[List[float]]:
    ...
    per-band pieces at that edge; otherwise return [box] unchanged.  The
    sweep's detector runs on unconfined tiles and can bridge a gutter,
    fusing two columns' text into one line.
    """
    x0, y0, x1, y1 = box
    cuts = sorted(set(
        round(c) for c in
...
def sweep_uncovered_text(
    ...
    """
    Recall safety-net: tile the page, detect lines at near-native resolution
    on the preprocessed page, and OCR every line not already covered by an
    existing element.

    - Boxes cut by an interior tile edge are ignored (the overlapping
      neighbour tile sees them whole).
    - `known` grows as lines are recovered, so tiles never re-read a line.
    - Lines straddling a column-band edge are split there first.
    - Recovered lines are read as "body" class from padded raw-render crops
      (no region scope: they have no region), with the stricter
      SWEEP_MIN_CONFIDENCE floor, labelled "Recovered", and given the
      nearest region's reading position; _assign_recovered_positions()
      relocates whole missed columns.
    """
    iw, ih = page_image.size
    m      = SWEEP_EDGE_MARGIN
    recovered: List[Dict] = []
    ...
    processor, model = recognizers["body"]
            ...
            for lx0, ly0, lx1, ly1 in _surya_line_bboxes(tile, det_predictor):
                ...
                for box in _split_wide_sweep_line(raw_box, column_bands):
                    ...
                    if _coverage(box, known, pad=SWEEP_COVERAGE_PAD) >= SWEEP_COVERED_FRAC:
                        continue
                    text, conf = _trocr_read(_recognition_crop(raw_image, box),
                                             processor, model,
                    ...
                    if _is_noise(text, conf, int(bh), int(bw), min_conf=SWEEP_MIN_CONFIDENCE):
                        ...
                        continue
                    elem = _make_element(text, box, conf, "Recovered",
                                         _nearest_position(box, layout_regions))
...
def _debug_path(stem: str, page_num: int, tag: str, ext: str) -> Path:
    """
    Path for one debug output file.  DEBUG_OVERWRITE → "latest_<tag>.<ext>",
    overwritten every page; otherwise per-page names.
    """
    if DEBUG_OVERWRITE:
...
def audit_coverage(
    ...
    Works at 1/AUDIT_SCALE size.  "Covered" = the boxes of all OCR elements
    plus skipped (Picture/Figure) regions.  Logs the uncovered share, lists
    wide mostly-uncovered vertical bands, and saves an uncovered-ink map.
    Returns the uncovered fraction (0–1).
    """
    s      = AUDIT_SCALE
...
def save_layout_report(regions: List[Dict], image_bbox: Optional[List[float]],
    ...
    lines = [
        ...
        "=" * 88,
        f"{'POS':>4}  {'LABEL':<22}  {'CLASS':<7}  {'X0':>6} {'Y0':>6} {'X1':>6} {'Y1':>6}"
...
def process_page(
    ...
    1.  Preprocess  — tiled contrast + unsharp mask (layout + body-line detection input)
    2.  Layout      — LayoutPredictor → labelled regions + reading-order positions
    2b. Reorder     — fix masthead regions Surya placed after the body
    3.  OCR         — each region read according to its label's recognition
                      class (see ocr_region / RECOGNITION_CLASSES)
    3b. Sweep       — OCR any detected line no region claimed
    3c. Dedupe      — drop duplicate reads of the same area
    4.  Sort        — Surya position, then visual row, then x
    4b. Beads       — one article-thread bead per reading-order block
    5.  Audit       — report printed ink still outside every OCR'd box
    ...
    # (i) A layout exception (typically accelerator out-of-memory) is retried
    # after freeing memory; if it persists the page is NOT processed with the
    # full-page fallback — that mixes every column into one reading position.
    layout_out = None
    for attempt in range(1, LAYOUT_ATTEMPTS + 1):
        try:
            ...
            logger.error(
                f"  LayoutPredictor failed (attempt {attempt}/{LAYOUT_ATTEMPTS}): {exc}"
            )
            free_accelerator_memory()
    else:
        raise LayoutFailedError(
            f"layout analysis failed on page {page_num} after {LAYOUT_ATTEMPTS} attempt(s)"
        )

    if layout_out:
        layout_regions, image_bbox = parse_layout_result(layout_out[0], processed.size)
        page_reported = getattr(layout_out[0], "page", "n/a")
        logger.debug(f"  [LAYOUT] result.page={page_reported}  image_bbox={image_bbox}")

    # Surya ran but found no regions (e.g. a near-blank page): read the page whole.
    ...
    # ── Stage 3: Per-region OCR, by recognition class ─────────────────────────
    text_regions    = [r for r in layout_regions if _label_class(r["label"]) != "skip"]
    unknown_regions = [r for r in text_regions if r["label"] not in LABEL_CLASS]
    if unknown_regions:
        unk_lbls = sorted({r["label"] for r in unknown_regions})
        logger.warning(
            f"  [LAYOUT] {len(unknown_regions)} region(s) with unrecognised "
            f"label(s) {unk_lbls} — reading as '{UNKNOWN_LABEL_CLASS}'."
        )
    class_counts = Counter(_label_class(r["label"]) for r in layout_regions)
    logger.info(
        "  [OCR] regions by class: "
        + "  ".join(f"{c}×{n}" for c, n in sorted(class_counts.items()))
    )
    ...
    for ri, region in enumerate(sorted(text_regions, key=lambda r: r["position"])):
        bbox = region["bbox"]
        logger.debug(
            f"    Region {ri+1}/{len(text_regions)}: {region['label']} "
            f"[{_label_class(region['label'])}] pos={region['position']}  "
            f"bbox=({bbox[0]:.0f},{bbox[1]:.0f}→{bbox[2]:.0f},{bbox[3]:.0f})"
        )
        all_elements.extend(
            ocr_region(processed, pil_image, region, det_predictor, recognizers)
        )
...
def insert_text_layer(
    ...
    For every line: font size from the line height; baseline at the box
    bottom raised by the font's descender; a horizontal morph matrix scales
    the string to span x0 → x1 of the box (clipped to the page).  Each line
    is written with its own write_text() call, in reading order.
    ELEMENT_SEPARATOR is appended to every line but excluded from the
    stretch, so it falls just past the line's right edge.
...
def process_pdf(
    ...
    Add an invisible OCR text layer to `input_path` and write `output_path`.
      ...
      1. Open the ORIGINAL PDF (image streams are never re-encoded).
      2. For each page: render → OCR → strip any pre-existing text layer →
         insert the new invisible text layer; collect article-thread beads.
      3. Write metadata and accessibility catalog entries.
      4. Subset the embedded font and save once with deflate.
      5. pikepdf pass: PDF/A OutputIntent + article threads; verify text.
            ...
            for idx in range(n_pages):
                ...
                try:
                    elements, beads = process_page(
                        ...
                        det_predictor, layout_predictor, recognizers,
                    )
                except LayoutFailedError:
                    raise
                except Exception as exc:
                    ...
                    elements, beads = [], []
                del pil_img
                free_accelerator_memory()      # (i) don't let the MPS/CUDA cache grow across pages

                # Strip any pre-existing text layer on EVERY page (image
                # streams untouched), so all output text comes from TrOCR.
    ...
    except LayoutFailedError as exc:
        logger.error(f"  {exc} — no output written; the file will be retried on the next run.")
        free_accelerator_memory()
        return False
    except Exception as exc:
...
def compress_to_target_size(input_pdf: Path, output_pdf: Path, original_size: int) -> Path:
    ...
    deflate only (images never re-encoded).  A result is accepted only if it
    fits the budget AND still contains extractable text.
    """
...
def main() -> None:
    ...
    for cls, spec in RECOGNITION_CLASSES.items():
        logger.info(
            f"  {cls:<9}: {spec['model']}  (detect on {spec['detect_on']}, "
            f"{'split wide lines' if spec['split_wide'] else 'read lines whole'})"
        )
    logger.info(
        f"  Recog.   : raw render, pad {PAD_X_EM}×{PAD_Y_EM} em (kept inside the region), "
    ...
    det_predictor, layout_predictor, recognizers = load_models()
