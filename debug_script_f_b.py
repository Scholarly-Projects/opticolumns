#!/usr/bin/env python3
"""
Opticolumns  –  debug_script_f_b.py
======================================================================
"""
...
# ── Text-quality safeguards ───────────────────────────────────────────────────
# Catch output the confidence floor cannot: decoder repetition loops (each
# repeated token is predicted with HIGH probability), fragment soup made of
# lone letters and digits, and text too long to physically fit its box.
REPEAT_UNIT_MAX     = 6      # longest repeating unit (chars) the loop detector looks for
REPEAT_MIN_COPIES   = 5      # a unit must repeat at least this many times in a row
REPEAT_MAX_FRAC     = 0.40   # reject if one repeating run covers more than this share of the text
MIN_CHAR_W_FRAC     = 0.20   # narrowest plausible avg glyph width, as a fraction of line
                             # height (~half of normal body type, so a line may hold ~2x the
                             # expected characters before it's treated as hallucinated)
MIN_WORDLIKE_FRAC   = 0.50   # at least this share of tokens must look like words or numbers
WORDLIKE_MIN_TOKENS = 3      # only judge word-likeness on lines with this many tokens
WORDLIKE_EXEMPT_LABELS = {"Table", "Form"}   # box scores, stock tables: digit soup is real
...
# ══════════════════════════════════════════════════════════════════════════════
# TEXT-QUALITY HELPERS  (cleanup + noise heuristics)
# ══════════════════════════════════════════════════════════════════════════════

_REPEAT_RE = re.compile(r"(.{1,%d}?)\1{%d,}" % (REPEAT_UNIT_MAX, REPEAT_MIN_COPIES - 1))
_VOWELS    = set("aeiouyAEIOUY")


def _repeat_frac(text: str) -> float:
    """
    Share of `text` covered by its longest run of one repeated unit
    (e.g. "S.S.S.S.S…", "the the the the the").  Catches TrOCR decoder
    loops, which the confidence floor misses because every repeated token is
    predicted with high probability.  Units made only of dots/dashes/spaces
    (table-of-contents leaders, rules) are ignored.
    """
    longest = 0
    for m in _REPEAT_RE.finditer(text):
        if m.group(1).strip(" .-_·"):
            longest = max(longest, m.end() - m.start())
    return longest / len(text) if text else 0.0


def _wordlike_frac(text: str) -> float:
    """
    Share of non-punctuation tokens that look like a word (2+ chars with a
    vowel) or a real number (2+ digits: prices, years, page numbers).
    Fragment soup such as "is 0 0 0 . rt. L. 0 0" scores very low.
    Lines with fewer than WORDLIKE_MIN_TOKENS tokens are not judged (1.0).
    """
    toks = [re.sub(r"[^\w$%]", "", t) for t in text.split()]
    toks = [t for t in toks if t]
    if len(toks) < WORDLIKE_MIN_TOKENS:
        return 1.0
    good = sum(
        1 for t in toks
        if len(t) >= 2 and (any(c in _VOWELS for c in t)
                            or sum(c.isdigit() for c in t) >= 2)
    )
    return good / len(toks)


_LEAD_JUNK   = re.compile(r'^(?:[#*|~_"]+\s+)+')      # detached junk at the start: '# by', '" concert'
_TRAIL_JUNK  = re.compile(r'(?:\s+[#*|~_"]+)+$')      # detached junk at the end:  'critics . "'
_PUNCT_SPACE = re.compile(r"\s+([.,;:!?])")


def _clean_text(text: str) -> str:
    """
    Strip DETACHED edge artefacts ('# by critics . "' → 'by critics.') and
    fix the IAM-style 'word .' spacing the handwritten checkpoint emits, so
    phrase search works.  Attached characters ('#1', '"We're') are left alone.
    """
    t = _LEAD_JUNK.sub("", text.strip())
    t = _TRAIL_JUNK.sub("", t)
...
def _trocr_read(
    ...
    The text is passed through _clean_text() before being returned.
    """
    try:
...
def _is_noise(
    ...
    line_level = True when (h, w) describe a single text line; the
    character-density check is only meaningful then.  Pass-2 whole-region
    reads, which may span several lines, pass False.

    label is the source layout label; regions in WORDLIKE_EXEMPT_LABELS
    (tables, forms) skip the word-likeness check, since box scores and
    stock quotes are legitimately made of lone digits.
    """
    if not text:
        return True
    ...
    if (tl > 3 and tc.isalpha()
            ...
            and confidence < 0.7):
        return True
    # Degenerate decoder loop: one short unit repeated across most of the line.
    if tl >= 12 and _repeat_frac(tc) > REPEAT_MAX_FRAC:
        return True
    # More characters than the box could physically hold (a hallucinated
    # continuation).  The mirror image of the sparse-width check below.
    if line_level and tl > w / (h * MIN_CHAR_W_FRAC):
        return True
    # Fragment soup: mostly single letters / lone digits.
    if (label not in WORDLIKE_EXEMPT_LABELS
            and _wordlike_frac(tc) < MIN_WORDLIKE_FRAC):
        return True
...
def ocr_region(
    ...
    for lb in line_bboxes:
        ...
        if _is_noise(text, confidence, lh, lw, label=label):
            logger.debug(
    ...
    # line_level=False: the whole region may span several lines, so the
    # character-density check does not apply.
    if not _is_noise(text_wb, conf_wb, rh, rw, line_level=False, label=label):
