...
# ── Coverage-gap second layout pass ──────────────────────────────────────────
# After the first LayoutPredictor call, strips of the page (most commonly
# the leftmost columns on dense historic newsprint) may receive no detections.
# The gap-fill pass projects existing detections onto the x-axis, locates
# horizontal strips with near-zero coverage, crops each one, and re-runs
# LayoutPredictor on the crop.  Results are offset back to page coordinates
# and merged with IoU-based deduplication.
#
# Tuning guide:
#   GAP_COV_THRESHOLD — lower = only reprocess heavily empty strips (fewer
#                       false re-runs); raise if columns are still missed
#   GAP_MIN_W_FRAC    — raise to skip hair-thin rule-line gaps; lower to
#                       catch narrow side-columns
#   GAP_CONTEXT_PX    — gives the crop a few pixels of neighbour context so
#                       the model can infer column boundaries; 20–40 is typical
GAP_COV_THRESHOLD = 0.10   # x-band is "uncovered" if < 10 % of page height detected
GAP_MIN_W_FRAC    = 0.04   # gap strip must span ≥ 4 % of page width
GAP_MIN_H_FRAC    = 0.10   # gap strip must span ≥ 10 % of page height (after y-clamp)
GAP_CONTEXT_PX    = 28     # pixels of overlap/context margin around each gap crop
GAP_IOU_THRESH    = 0.25   # IoU above which a new candidate region is a duplicate
GAP_SCALE         = 8      # downsample factor for the coverage mask (8 px per cell)
...
def _gap_fill_pass(
    ...
    """
    Second-pass layout detection that targets uncovered page strips.

    APPROACH
    --------
    1. Build an x-axis coverage projection by downsampling the page to
       GAP_SCALE px-per-cell and marking every cell touched by an existing
       bbox.  The projection is the fraction of page height covered at each
       x position.

    2. Find contiguous x-ranges where that fraction is below GAP_COV_THRESHOLD.
       These are strips where the first pass returned nothing.

    3. For each qualifying strip, determine the y-extent of uncovered cells,
       add GAP_CONTEXT_PX of overlap on all four sides, and crop the image.

    4. Re-run LayoutPredictor on the crop.  Shift returned bbox coordinates
       back to full-page space.

    5. Discard any new region whose IoU with an existing region exceeds
       GAP_IOU_THRESH (prevents double-counting at overlap margins).

    6. After merging, re-sort ALL regions by (estimated column band, y-centre)
       and re-assign sequential position values so that gap-fill additions
       appear in their natural column-first reading order rather than always
       sorting after originally detected regions.

    Returns the merged, re-positioned region list.
    """
    iw, ih = image.size
    scale  = GAP_SCALE
    mw     = math.ceil(iw / scale)
    mh     = math.ceil(ih / scale)

    # ── 1. Build downsampled coverage mask ───────────────────────────────────
    # covered[mx] = set of mask-row indices (my) touched by at least one bbox
    covered: List[set] = [set() for _ in range(mw)]
    for r in regions:
        bx0, by0, bx1, by1 = r["bbox"]
        mx0 = max(0, int(bx0 / scale))
        mx1 = min(mw, math.ceil(bx1 / scale))
        my0 = max(0, int(by0 / scale))
        my1 = min(mh, math.ceil(by1 / scale))
        for mx in range(mx0, mx1):
            covered[mx].update(range(my0, my1))

    cov_frac = [len(covered[mx]) / mh for mx in range(mw)]

    # ── 2. Find uncovered x-strips ────────────────────────────────────────────
    min_w_cells = max(1, math.ceil(iw * GAP_MIN_W_FRAC / scale))
    gaps_px: List[Tuple[int, int]] = []   # (x_start, x_end) in page pixels

    in_gap    = False
    gap_start = 0
    for mx, cf in enumerate(cov_frac):
        if cf < GAP_COV_THRESHOLD:
            if not in_gap:
                in_gap    = True
                gap_start = mx
        else:
            if in_gap:
                if mx - gap_start >= min_w_cells:
                    gaps_px.append((gap_start * scale, min(mx * scale, iw)))
                in_gap = False
    if in_gap and mw - gap_start >= min_w_cells:
        gaps_px.append((gap_start * scale, iw))

    if not gaps_px:
        return regions

    logger.info(
        f"  [GAP PASS] {len(gaps_px)} uncovered strip(s): "
        + ", ".join(f"x={g[0]}–{g[1]}" for g in gaps_px)
    )

    new_regions = list(regions)

    for gx0, gx1 in gaps_px:
        # ── 3. Find y-extent of uncovered cells in this x-strip ───────────────
        mx0_g = max(0, int(gx0 / scale))
        mx1_g = min(mw, math.ceil(gx1 / scale))
        all_rows   = set(range(mh))
        strip_rows = set()
        for mx in range(mx0_g, mx1_g):
            strip_rows.update(all_rows - covered[mx])

        if strip_rows:
            gy0 = min(strip_rows) * scale
            gy1 = min((max(strip_rows) + 1) * scale, ih)
        else:
            gy0, gy1 = 0, ih

        if (gy1 - gy0) < ih * GAP_MIN_H_FRAC:
            logger.debug(
                f"  [GAP PASS] Strip x={gx0}–{gx1}: "
                f"y-extent {gy1-gy0}px < minimum — skipping."
            )
            continue

        # Add context margin so the model sees column neighbours
        cx0 = max(0,  gx0 - GAP_CONTEXT_PX)
        cy0 = max(0,  gy0 - GAP_CONTEXT_PX)
        cx1 = min(iw, gx1 + GAP_CONTEXT_PX)
        cy1 = min(ih, gy1 + GAP_CONTEXT_PX)

        crop = image.crop((cx0, cy0, cx1, cy1))
        logger.info(
            f"  [GAP PASS] Re-running layout on "
            f"({cx0},{cy0}→{cx1},{cy1})  {crop.width}×{crop.height}px"
        )

        # ── 4. Re-run LayoutPredictor on the crop ─────────────────────────────
        try:
            gap_out = layout_predictor([crop])
        except Exception as exc:
            logger.warning(f"  [GAP PASS] LayoutPredictor failed: {exc}")
            continue

        if not gap_out:
            continue

        gap_regions, _ = parse_layout_result(gap_out[0])
        logger.info(
            f"  [GAP PASS]   → {len(gap_regions)} candidate region(s)."
        )

        # ── 5. Offset coordinates, deduplicate, merge ─────────────────────────
        added = 0
        for gr in gap_regions:
            bx0, by0, bx1, by1 = gr["bbox"]
            abs_bbox = [
                max(0.0,  bx0 + cx0),
                max(0.0,  by0 + cy0),
                min(float(iw), bx1 + cx0),
                min(float(ih), by1 + cy0),
            ]
            if abs_bbox[2] - abs_bbox[0] < MIN_REGION_W:
                continue
            if abs_bbox[3] - abs_bbox[1] < MIN_REGION_H:
                continue
            if any(_iou(abs_bbox, er["bbox"]) > GAP_IOU_THRESH for er in new_regions):
                logger.debug(
                    f"  [GAP PASS]   Duplicate: {gr['label']} "
                    f"bbox={[round(v) for v in abs_bbox]}"
                )
                continue
            gr_copy         = dict(gr)
            gr_copy["bbox"] = abs_bbox
            new_regions.append(gr_copy)
            added += 1
            logger.debug(
                f"  [GAP PASS]   Added: {gr_copy['label']} "
                f"bbox={[round(v) for v in abs_bbox]}"
            )

        if added:
            logger.info(f"  [GAP PASS]   Accepted {added} new region(s) from this strip.")

    n_added = len(new_regions) - len(regions)
    if not n_added:
        return regions   # nothing changed

    logger.info(f"  [GAP PASS] Total new regions added: {n_added}")

    # ── 6. Re-sort and re-assign positions ────────────────────────────────────
    # Estimate column width from the average region width; use it to bin
    # x-centroids into column bands.  Within each band sort top-to-bottom.
    # This preserves Surya's intra-column ordering while correctly placing
    # gap-fill additions in their natural left-to-right column sequence.
    avg_w  = sum(r["bbox"][2] - r["bbox"][0] for r in new_regions) / len(new_regions)
    col_w  = max(avg_w, iw / 12.0)   # floor: never narrower than 1/12 page

    new_regions.sort(key=lambda r: (
        round(((r["bbox"][0] + r["bbox"][2]) / 2.0) / col_w),  # column band
        (r["bbox"][1] + r["bbox"][3]) / 2.0,                   # y-centroid
    ))
...
def process_page(
    ...
    # ── Stage 2b: Coverage-gap second layout pass ─────────────────────────────
    # Re-runs LayoutPredictor on any page strip not covered by Stage 2.
    # Typical beneficiary: leftmost columns on dense historic newsprint where
    # the model's first pass returns nothing due to tight column spacing or
    # uneven ink density at page margins.
    n_before_gap = len(layout_regions)
    layout_regions = _gap_fill_pass(processed, layout_regions, layout_predictor)
    if len(layout_regions) > n_before_gap:
        save_layout_debug(
            pil_image, layout_regions,
            Path(str(pfx) + "_01b_layout_gap.jpg"),
        )
        logger.info(
            f"  [GAP PASS] Layout augmented: "
            f"{n_before_gap} → {len(layout_regions)} regions."
        )
