#!/usr/bin/env python3
"""
Opticolumns  –  debug_script_d.py
======================================================================
Changes from previous revision:
  - load_models()       : LAYOUT_BATCH_SIZE / DETECTOR_BATCH_SIZE lowered to 4
                          for denser per-image attention on historic broadsheets;
                          Surya settings object patched immediately after env vars.
  - _LABEL_ALIAS        : Extended with snake_case variants, caption aliases, and
                          paragraph/body synonyms seen in newer Surya checkpoints.
  - parse_layout_result : Now accepts orig_image_size and applies coordinate-space
                          rescaling via image_bbox (fixes off-page bbox coordinates
                          at 300 DPI); added _iou() + _nms_regions() to suppress
                          duplicate detections on wide column strips.
  - process_page        : Passes processed.size to parse_layout_result.
======================================================================
"""
...
def load_models():
    ...
    Environment variables tuned here for historic newsprint:
        LAYOUT_BATCH_SIZE    — controls GPU/CPU memory per batch item (~220 MB VRAM each).
                               Lower = more careful per-image attention; default 32 is too
                               aggressive for dense multi-column scans.
    ...
    # ── Surya performance / sensitivity tuning ────────────────────────────────
    # For dense historic newspaper columns the default batch sizes cause the
    # layout backbone to rush through crops that need careful attention.
    # Lower batch sizes give each image more compute budget per forward pass.
...
def _nms_regions(regions: List[Dict], iou_threshold: float = 0.45) -> List[Dict]:
    ...
    box wins when two boxes substantially overlap.
    """
    if not regions:
        return regions
    sorted_r = sorted(
    ...
    for i, r in enumerate(sorted_r):
        ...
        for j in range(i + 1, len(sorted_r)):
            if j in suppressed:
                continue
            if _iou(r["bbox"], sorted_r[j]["bbox"]) > iou_threshold:
                suppressed.add(j)
    return kept
