#!/usr/bin/env python3
"""
Opticolumns - Historic Newspaper OCR Pipeline
======================================================================
Enhanced with auxiliary layout analysis for multi-column historic newspapers
"""
import sys
import os
import re
...
MIN_REGION_W = 40
MIN_REGION_H = 15
# ── Auxiliary Layout Analysis Settings ────────────────────────────────────────
# Enable secondary layout pass for historic newspapers with missed columns
ENABLE_AUXILIARY_LAYOUT = True
# Minimum gap size between detected regions to trigger auxiliary analysis (pixels)
MIN_GAP_WIDTH = 100
MIN_GAP_HEIGHT = 200
# Maximum number of auxiliary regions to add per page (prevent over-segmentation)
MAX_AUXILIARY_REGIONS = 10
# Confidence threshold for auxiliary region acceptance
AUXILIARY_CONFIDENCE_THRESHOLD = 0.15
...
LABEL_COLOURS: Dict[str, str] = {
    ...
    "Auxiliary-Text":    "#FF9800",  # New colour for auxiliary detections
}
...
def parse_layout_result(result) -> Tuple[List[Dict], Optional[List[float]]]:
    ...
    for box in result.bboxes:
        ...
        regions.append({
            ...
            "position": position,
            "top_k":    top_k,
            "auxiliary": False,  # Mark as primary detection
        })
    return regions, image_bbox
# ══════════════════════════════════════════════════════════════════════════════
# AUXILIARY LAYOUT ANALYSIS
# ══════════════════════════════════════════════════════════════════════════════
def _find_text_gaps(
    regions: List[Dict],
    page_width: int,
    ...
    """
    Identify gaps between detected text regions that likely contain missed columns.
    Uses horizontal and vertical gap analysis based on newspaper column structure.
    Returns list of potential auxiliary regions.
    """
    if not regions:
        return []
    ...
    for region in sorted_regions:
        ...
        covered_y_ranges.append((y0, y1))
    # Find vertical gaps (potential missed columns)
    covered_y_ranges.sort()
    merged_ranges = []
    for start, end in covered_y_ranges:
    ...
    # For each vertical gap, check horizontal coverage
    for gap in gap_candidates:
        # Find regions in adjacent rows to estimate column width
        adjacent_regions = [
            r for r in sorted_regions
            if abs(r["bbox"][1] - gap["y0"]) < 100 or abs(r["bbox"][3] - gap["y1"]) < 100
        ]
        if not adjacent_regions:
            continue
        # Estimate typical column width from adjacent regions
        column_widths = [r["bbox"][2] - r["bbox"][0] for r in adjacent_regions]
        if not column_widths:
            continue
        median_col_width = sorted(column_widths)[len(column_widths) // 2]
        # Create auxiliary regions for uncovered horizontal spaces
        covered_x_ranges = []
        for region in adjacent_regions:
            if gap["y0"] <= region["bbox"][1] <= gap["y1"] or gap["y0"] <= region["bbox"][3] <= gap["y1"]:
                x0, x1 = region["bbox"][0], region["bbox"][2]
                covered_x_ranges.append((x0, x1))
        covered_x_ranges.sort()
        # Find horizontal gaps
        current_x = 0
        for x0, x1 in covered_x_ranges:
            if x0 - current_x >= MIN_GAP_WIDTH:
                # Found a gap - create auxiliary region
                aux_region = {
                    "bbox": [float(current_x), float(gap["y0"]), float(x0), float(gap["y1"])],
                    "polygon": None,
                    "label": "Text",
                    "position": 9999,  # Will be reassigned
                    "top_k": {},
                    "auxiliary": True,
                    "confidence": AUXILIARY_CONFIDENCE_THRESHOLD,
                }
                auxiliary_regions.append(aux_region)
            current_x = max(current_x, x1)
        # Check for gap at right edge
        if page_width - current_x >= MIN_GAP_WIDTH:
            aux_region = {
                "bbox": [float(current_x), float(gap["y0"]), float(page_width), float(gap["y1"])],
                "polygon": None,
                "label": "Text",
                "position": 9999,
                "top_k": {},
            ...
            auxiliary_regions.append(aux_region)
    # Limit number of auxiliary regions
    if len(auxiliary_regions) > MAX_AUXILIARY_REGIONS:
        # Keep largest regions by area
        auxiliary_regions.sort(
            key=lambda r: (r["bbox"][2] - r["bbox"][0]) * (r["bbox"][3] - r["bbox"][1]),
            reverse=True
        )
        auxiliary_regions = auxiliary_regions[:MAX_AUXILIARY_REGIONS]
    return auxiliary_regions
def _run_auxiliary_layout_analysis(
    page_image: Image.Image,
    initial_regions: List[Dict],
    layout_predictor: LayoutPredictor,
) -> List[Dict]:
    """
    Run secondary layout analysis on areas not covered by initial detection.
    This catches missed columns in historic multi-column newspapers.
    """
    if not ENABLE_AUXILIARY_LAYOUT:
        return []
    page_width, page_height = page_image.size
    # Find gaps in initial detection
    gap_regions = _find_text_gaps(initial_regions, page_width, page_height)
    if not gap_regions:
        logger.debug("  [AUX] No significant gaps detected")
        return []
    logger.info(f"  [AUX] Found {len(gap_regions)} potential gap regions")
    # Run layout predictor on gap regions to validate they contain text
    auxiliary_regions = []
    for gap in gap_regions:
        x0, y0, x1, y1 = [int(v) for v in gap["bbox"]]
        # Skip if too small
        if (x1 - x0) < MIN_REGION_W or (y1 - y0) < MIN_REGION_H:
            continue
        # Crop the gap area
        gap_crop = page_image.crop((x0, y0, x1, y1))
        # Run layout analysis on the gap
        try:
            gap_result = layout_predictor([gap_crop])
            if gap_result and hasattr(gap_result[0], "bboxes"):
                for box in gap_result[0].bboxes:
                    if hasattr(box, "bbox") and box.bbox:
                        bbox = [float(v) for v in box.bbox]
                        ...
                        raw_label = getattr(box, "label", "Text")
                        label = _normalise_label(raw_label)
                        # Always label as "Text" for auxiliary regions (per requirements)
                        aux_region = {
                            "bbox": abs_bbox,
                            "polygon": None,
                            "label": "Text",
            ...
            logger.debug(f"    [AUX] Layout analysis on gap failed: {exc}")
            # If layout analysis fails, still add the gap as a text region
            # (better to OCR too much than miss content)
            aux_region = {
            ...
            auxiliary_regions.append(aux_region)
    logger.info(f"  [AUX] Added {len(auxiliary_regions)} auxiliary text regions")
    return auxiliary_regions
# ══════════════════════════════════════════════════════════════════════════════
...
def save_layout_debug(image: Image.Image, regions: List[Dict], path: Path) -> None:
    ...
    for region in sorted(regions, key=lambda r: r.get("position", 999)):
        ...
        tag   = f"[{pos}] {label}" + ("*" if is_aux else "")
        tag_w = len(tag) * 9 + 6
...
def save_layout_report(regions: List[Dict], image_bbox: Optional[List[float]],
    ...
    for r in sorted(regions, key=lambda x: x.get("position", 999)):
        ...
        lines.append(
            ...
            f"  {top_k_str}  {'YES' if is_aux else 'NO':>3}"
        )
...
def process_page(
    ...
    3. Auxiliary   — Gap analysis + secondary layout on missed areas
    4. OCR         — DetectionPredictor (line segmentation) + TrOCR per line
    5. Sort        — by Surya layout position, then vertical baseline within region
    ...
    save_layout_report(layout_regions, image_bbox,
                       ...
                       filename, page_num)
    # ── Stage 3: Auxiliary Layout Analysis ────────────────────────────────────
    auxiliary_regions = []
    if ENABLE_AUXILIARY_LAYOUT and layout_regions:
        logger.info("  [AUX] Running auxiliary layout analysis …")
        auxiliary_regions = _run_auxiliary_layout_analysis(
            processed, layout_regions, layout_predictor
        )
        if auxiliary_regions:
            # Assign positions to auxiliary regions (after primary regions)
            max_position = max(r["position"] for r in layout_regions) if layout_regions else 0
            for i, aux in enumerate(auxiliary_regions):
                aux["position"] = max_position + i + 1
            # Merge auxiliary regions with primary regions
            layout_regions.extend(auxiliary_regions)
            logger.info(f"  [AUX] Total regions after merge: {len(layout_regions)}")
            # Save updated layout debug with auxiliary regions marked
            save_layout_debug(pil_image, layout_regions,
                              Path(str(pfx) + "_01_layout_with_aux.jpg"))
            save_layout_report(layout_regions, image_bbox,
                               Path(str(pfx) + "_01_layout_report_with_aux.txt"),
                               filename, page_num)
    # ── Stage 4: Per-region DetectionPredictor + TrOCR ────────────────────────
    ...
    # Count auxiliary elements
    aux_count = sum(1 for e in all_elements if e.get("auxiliary", False))
    if aux_count > 0:
        logger.info(f"  [AUX] {aux_count} element(s) from auxiliary analysis")
    if all_elements:
...
def main() -> None:
    ...
    logger.info("║  + Auxiliary Layout Analysis for Historic Newspapers        ║")
    logger.info("╚══════════════════════════════════════════════════════════════╝")
    ...
    logger.info(f"  Aux Layout: {ENABLE_AUXILIARY_LAYOUT}")
    logger.info("")
